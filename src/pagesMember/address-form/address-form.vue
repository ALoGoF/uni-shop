...
onLoad(() => {
  if (query.id) {
    getMemberAddressById(query.id)
  }
  uni.setNavigationBarTitle({ title: query.id ? '修改地址' : '新增地址' })
})

let regionCode: [string, string, string] = ['', '', '']
const handleAddressChange: UniHelper.RegionPickerOnChange = (ev) => {
  const {
    detail: { value, code },
  } = ev
  regionCode = code!
...
const handleDefaultChange: UniHelper.SwitchOnChange = (ev) => {
  const {
    detail: { value },
  } = ev
...
const handleSubmit = async () => {
  ...
  if (query.id) {
    await putMemberAddressApi(form.value, query.id)
  } else {
    await postMemberAddressApi(form.value)
  }
  uni.showToast({
    icon: 'success',
    title: query.id ? '地址修改成功' : '地址添加成功',
  })
  setTimeout(() => {
    uni.navigateBack()
  }, 400)
